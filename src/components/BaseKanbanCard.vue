<template>
  <div class="card-row" :class="statusClass(card.status)" @dblclick="$emit('edit', card)">
    <span class="title">{{ card.title }}</span>

    <span class="icons" @mousedown.stop @click.stop>
      <a-tag v-if="card.status === 'In progress' && card.priority"
        :style="{ backgroundColor: getPriorityColor(card.priority), color: '#fff' }" class="priority-tag">
        {{ card.priority }}
      </a-tag>
